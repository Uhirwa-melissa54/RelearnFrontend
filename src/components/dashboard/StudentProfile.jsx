...
import { studentApi, getUser, performLogout } from '../../api';
      ...
      <div className="profile-grid">
        ...
        <div className="profile-info-card card-box">
          <div className="profile-avatar-large">
            {(displayProfile?.fullName || 'S').charAt(0).toUpperCase()}
          </div>
          <h2>{displayProfile?.fullName || 'Student'}</h2>
          ...
          <div className="info-list">
            ...
            <div className="info-item">
              ...
              <span>Role: {displayProfile?.role || 'STUDENT'}</span>
            </div>
            {displayProfile?.className && (
              <div className="info-item" style={{ padding: '12px', backgroundColor: '#f9fafb', borderRadius: '8px', justifyContent: 'center' }}>
                <strong>Class:</strong>&nbsp;{displayProfile.className}
              </div>
            )}
            {displayProfile?.academicYear && (
              <div className="info-item" style={{ padding: '12px', backgroundColor: '#f9fafb', borderRadius: '8px', justifyContent: 'center' }}>
                <strong>Academic Year:</strong>&nbsp;{displayProfile.academicYear}
              </div>
            )}
            {displayProfile?.createdAt && (
              <div className="info-item" style={{ padding: '12px', backgroundColor: '#f9fafb', borderRadius: '8px', justifyContent: 'center' }}>
                <strong>Joined:</strong>&nbsp;{new Date(displayProfile.createdAt).toLocaleDateString()}
              </div>
            )}
          </div>
